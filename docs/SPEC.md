...
- Phase 2.1: the badge hides while the band is on screen, collapses to its tab (or hides) when a fixed or sticky host element (chat button, cookie bar) is under it, and counts a view only when its open card is visible. On phones it starts as a pill. Minimized state (7 days) and the rotation counter live in the host's localStorage. z-index 900: above typical sticky headers, below most modals.
